...
set_power_mode()
{
    local mode="$1"
    local app="$2"
    if [ ! "$scene_current_mode" = "$mode" ]; then
...
trigger_scene()
{
    ...
    for list_app in ${performance[@]}; do
        if [ "$list_app" = "$app" ]; then
            set_power_mode "performance" "$app"
            return
        fi
    done
...
watcher_start()
{
    ...
    am monitor | while read line; do
        if [ "$line" = *Activity* ]; then
            local current_app="$(echo "$line" | cut -f2 -d ':')"
